...

{
  ...
  # Define whatever tailscale is enabled or not
  services.tailscale.enable = tailscale;

  # 🛡️ BASE FIREWALL
  # Guest blocking logic is inside modules/guest.nix
  networking.firewall = lib.mkIf tailscale {
    allowedUDPPorts = [ 41641 ]; # Tailscale port
    trustedInterfaces = [ "tailscale0" ]; # Tailscale interface
    checkReversePath = "loose"; # Required for Tailscale to function properly behind NAT
  };
}
