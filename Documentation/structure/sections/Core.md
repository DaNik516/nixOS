...
* **Global Variables:** Defines host-specific settings (like username, monitor layout, and wallpapers) that are passed down to all modules via `specialArgs` and `extraSpecialArgs`.
...
Each hosts folder should contain these 4 files
...
* ### `flatpak.nix` ###
* Defines which flatpak apps to install. It is safe to keep since it is ignored if the variable is disabled in `flake.nix`
